 ...
 * 
 * By default, draw the entire domain of the rectangle, otherwise you can pass a filtering function to only write our
*/
void funcToObj(
    ...
    const float ymin, 
    const float ymax,
    const std::function<bool (float, float)> inDomain
) {
    ...
    for (int i = 0; i < numSamplesDir; ++i) {
        for (int j = 0; j < numSamplesDir; ++j) {
            if(inDomain(minX + i * delX, minY + j * delY)) {
